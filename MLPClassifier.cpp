...
double_tensor MLPClassifier::predict(double_tensor X, bool make_decision){
    ...
    //YOUR CODE IS HERE
    // double_tensor Y = xt::zeros_like(X);
...
//     // double_tensor results = xt::zeros<double>({total_batch, nClasses});
//     double_tensor results;
//     size_t total_samples = 0;
//     for(auto batch: *pLoader){
//         //YOUR CODE IS HERE
//         double_tensor X_batch = batch.getData();
//         double_tensor Y_batch = X_batch;
//         for (auto& layer : this->m_layers) {
//             Y_batch = layer->forward(Y_batch);
//         }
//         if (first_batch) {
//             results = xt::zeros<double>(Y_batch.shape());
//             results = Y_batch;
//             first_batch = false;
//         } else {
//             results = xt::concatenate(xt::xtuple(results, Y_batch), 0);
//         }    

//         // Update counters and print info
//         nsamples += Y_batch.shape()[0];
...
double_tensor MLPClassifier::predict(// Mine
    DataLoader<double, double>* pLoader,
    bool make_decision) {
    ...
    bool first_batch = true;
    for (auto batch : *pLoader) {
        double_tensor Y_batch = batch.getData();
        for (auto layer : this->m_layers) {
            Y_batch = layer->forward(Y_batch);
        }
        auto batch_size = Y_batch.shape()[0];
        if (first_batch) {
            auto shape = Y_batch.shape();
            shape[0] = pLoader->get_sample_count();  
            results = xt::zeros<double>(shape);  
            xt::view(results, xt::range(0, Y_batch.shape()[0]), xt::all()) = Y_batch;
            first_batch = false;
        }
        else{
            xt::view(results, xt::range(nsamples, nsamples + batch_size), xt::all()) = Y_batch;
        }
        nsamples += batch_size;
        cout << fmt::format("{:<6d}/{:<12d}|{:<50d}\n", batch_idx, total_batch, nsamples);
        batch_idx++;
...
double_tensor MLPClassifier::evaluate(DataLoader<double, double>* pLoader){//Mine
    bool old_mode = this->m_trainable;
    this->set_working_mode(false);
    ...
    auto metrics = meter.get_metrics();
...
double_tensor MLPClassifier::forward(double_tensor X){// Y = forward(X)
    for (auto hidden_layer : m_layers){
